...
//New handle rings function - builds a nested tree of polygons containing other polygons...
function handleRings(rings) {
  // Step 1: Build nesting tree
  ...
  for (const root of tree) {
    rewindRingTree(root); // enforce alternating winding
    polygons.push(...flattenTree(root));
  }
...
function buildRingTree(rings) {
  const processed = rings.map(r => isClockWise(r));
  const roots = [];

  for (const candidate of processed) {
    let parent = null;

    for (const possibleParent of processed) {
      if (candidate === possibleParent) continue;
      if (contains(possibleParent, candidate)) {
        // ✅ Only attach as child if winding differs
        if (candidate.clockWise !== possibleParent.clockWise) {
          // choose the closest parent (smallest container)
          if (!parent || contains(parent, candidate)) {
    ...
    if (parent) {
      ...
      roots.push(candidate); // same winding → promote to root
    }
  }
...
function rewindRingTree(node, depth = 0) {
  const desiredClockwise = depth % 2 === 1; //odd = outer, even = hole
  if (node.clockWise !== desiredClockwise) {
    node.ring = reverseRing(node.ring);
    node.clockWise = desiredClockwise;
  }
  for (const child of node.children) {
    rewindRingTree(child, depth + 1);
  }
}
...
// ---------------------
// Flatten into GeoJSON polygons
// ---------------------

function flattenTree(node, depth = 0) {
  if (depth % 2 === 0) {
    // Outer: build polygon [outer, hole1, hole2, ...]
    const holes = [];
    const extraPolygons = [];
    ...
    for (const child of node.children) {
      if ((depth + 1) % 2 === 1) {
        ...
        // hole's children (depth 2) → promoted to new root polygons
        for (const grandChild of child.children) {
          extraPolygons.push(...flattenTree(grandChild, depth + 2));
        }
      }
    }

    return [[node.ring, ...holes], ...extraPolygons];
  } else {
    // should never start at odd depth
    return [];
  }
}
